# Pallas & Minerva — Curriculumoverzicht (referentie voor Chronica Classica)

Dit document is een **naslagwerk**, geen masterplan — het bestaat om snel te
kunnen controleren of Chronica's hoofdstukindeling nog op een goed spoor zit
qua grammatica-tempo en -balans tussen Grieks (Pallas) en Latijn (Minerva).
Bronnen: de docentenboeken (`Pallas/!Pallas 1/2 Docentenboek...pdf`,
`Minerva/!Minerva-1/2-Docentenboek-v2022.pdf`) — geen theorietekst, maar de
per-paragraaf oefeningtitels (`Taaloefening X (par. N, onderwerp)`) geven
betrouwbaar aan wat een hoofdstuk/les daadwerkelijk NIEUW introduceert.

**Kernprincipe (staand beleid, zie Chronica.md
§7.38/§7.41-7.44)**: Chronica kiest grammatica uit de volledige Pallas/
Minerva-checklist op basis van wat pedagogisch aan de beurt is, LOSGEKOPPELD
van welk Pallas-les/Minerva-hoofdstuk toevallig bij welk Chronica-verhaal
past. De kolom "Chronica" hieronder toont dus waar de GRAMMATICA is
ingezet, niet per se waar het bijbehorende cultuurtekst-verhaal in Chronica
wordt naverteld (die twee lopen bewust vaak uiteen).

---

## Deel 1 — Pallas 1 (les 1-14) & Minerva 1 (hoofdstuk 1-13)

Gebruikt in Chronica Hoofdstuk 1-9 (proloog t/m Ilion in Vlammen). Minerva
deel 1 is volledig afgerond bij Hoofdstuk 9 (zie §7.38, vijfde pas); Pallas
deel 1 was dat al vanaf het begin.

| Pallas les | Onderwerp | Minerva H | Onderwerp | Chronica H |
|---|---|---|---|---|
| — | (introductielessen, alfabet) | H1 | Het Latijn en de Romeinen (intro) | Proloog |
| 2 | De Griekse goden | H2 | Midas: van koning tot ezel | H1 |
| 3 | De held Herakles (werken 1) | H3 | Latona: een wrekende godin | H2 |
| 4 | Herakles (werken 2, "gevaarlijke opdrachten") | H4 | Jupiter en Io: een goddelijke affaire | H2/H3 |
| 5 | Apotheose van Herakles | H5 | Phaëthon: een menselijke ruiter | H3 |
| 6-7 | Theseus, Ariadne, Minotauros, terugreis | H6 | De Familia (Titus Manlius, Coriolanus — Romeinse legenden) | H4 |
| — | *(herhaling, valt buiten lesnummering)* | H7 | Slavernij | H5, H9 (grammatica) |
| — | *(herhaling)* | H8 | Onderwijs | H6, H8 (grammatica) |
| — | *(herhaling)* | H9 | Het ontstaan van de mens (Deucalion & Pyrrha) | H7, H5 (grammatica, M9-deel) |
| 8-11 | Expeditie naar Troje, wrok van Achilles, Patroklos, wraak en medelijden | H10 | Aeneas (het houten paard — A.C.I.) | H8, H9 (grammatica) |
| 12-14 | Ajax, ondergang van Troje, Griekse kunst/mythe-receptie | H11 | Het begin van Rome (Hercules/Euander/Cacus; Romulus/Remus — **let op:** cultuurtekst hier is dezelfde Hercules&Cacus/Romulus&Remus-stof die Chronica H12 vertelt, maar de GRAMMATICA van dit hoofdstuk — passief, deponentia — is al verplaatst naar Chronica H6/H7) | H9, H6/H7 (grammatica) |
| — | — | H12 | Tirannen en vrienden (Rome na Romulus, macht v/d koning — betrekkelijk vnw, conjunctivus passief) | H7 (grammatica) |
| — | — | H13 | Waarheid en wijsheid (**pure herhaling**, geen nieuwe grammatica) | — |

**Grammatica-samenvatting H1-9** (al volledig verwerkt, zie
`SP_CAMPAIGN` ch1-ch9 in `singleplayer-data.js` voor het exacte, geaudite
detail): zn/bn groep 1-2, naamvallen nom./gen./dat./acc./abl./voc.,
praesens/imperfectum/perfectum/plusquamperfectum, infinitivus,
imperativus, esse/posse, 3e-declinatie (medeklinker- en i-stammen),
comparativus/superlativus, aanwijzend/persoonlijk voornaamwoord,
A.C.I., congruentie, passief (praesens/perfectum), deponentia,
betrekkelijk voornaamwoord, conjunctivus in de bijzin (cum-historicum/
-iterativum), conjunctivus passief.

---

## Deel 2 (begonnen) — Pallas 2 (les 15-21) & Minerva 2 (hoofdstuk 14-17)

Gebruikt in Chronica Hoofdstuk 10-12. Sinds de H12/13-samenvoeging
(§7.44) draagt Hoofdstuk 12 zowel Minerva 16 als 17.

| Pallas les | Onderwerp | Grammatica | Minerva H | Onderwerp | Grammatica | Chronica H |
|---|---|---|---|---|---|---|
| 15 | Odysseus bij de Faiaken (Nausikaä) | Medium | 14 | Rome: multiculturele samenleving (Abraham/Izaäk) | Naamwoordgroep 4/5 (manus/dies); **hic** (NIET ille — dat is al H8-stof, zie §7.41), ipse | H10 |
| 16 | De Kykloop Polyfemos | Passief, aoristus passief -θην | — | — | — | H10 |
| 17 | Kirke, Odysseus ontmoet Hermes | Participium | 15 + **18** | Samson en Judith (H15); Hannibal verslagen (H18, alleen de ablativus absolutus-paragraaf, par. I-III, vooruitgehaald) | Participium praesens actief (ppa, H15) + ablativus absolutus (H18) | H11 |
| 18 | Odysseus ontmoet zijn moeder (onderwereld), Achilles/Ajax | Genitivus absolutus | — | — | — | H11 |
| 19 | Odysseus op Ithaka (Athena, Penelope, Eurykleia) | Betrekkelijk voornaamwoord | 16 | Dieren in het Romeinse Rijk (raaf, leeuw) | Bijwoord op -iter, genitivus op -ius (9 onregelmatige bn), idem, ACP (accusativus cum participio) | H12 |
| 20-21 | **Boogwedstrijd, wraak op de vrijers, hereniging Penelope** ("als een rund bij zijn ruif", Electra-verwijzing) | Conjunctivus, alpha-werkwoorden | 17 | Hannibal over de Alpen (de eed van Hannibal, de tocht) — **cultuurtekst niet gevolgd**, grammatica (futurum/futurum exactum/ire) via Romulus & Remus verteld, zie decouplingsbeleid hierboven | Futurum, futurum exactum, het werkwoord ire | H12 — **gebouwd 2026-08-03** (les 19 = CH12_ODY_004, les 20-21 = CH12_ODY_006/009; Minerva 17 = CH12_LAT_006-014, incl. ut-finalis uit Minerva 14 par. IV) |

**Correctie gevonden bij dit onderzoek, direct hersteld (2026-08-02)**: de
eerdere Chronica-audit (§7.41) plaatste "ablativus absolutus" ten onrechte
bij Minerva hoofdstuk 15 (participium). Uit deze volledige doorlichting
bleek de ablativus absolutus pas in **Minerva hoofdstuk 18** ("abl. abs.,
vormleer en betekenis", par. I-III) als nieuw te worden geïntroduceerd —
H15 gaat uitsluitend over het participium (ppa/ppp). De puzzel zelf
(`puzzle_ch11_aen_ablativus_absolutus`, Chronica H11) bleef staan — hij is
taalkundig prima en past goed bij Mercurius' scène — alleen het
bronetiket is gecorrigeerd naar "Minerva 18, bewust vooruitgehaald" in
`SP_CAMPAIGN.ch11`, de bijbehorende codex-teksten en de puzzel-comments in
`singleplayer-data.js`. Zie ook `codex_grammatica_ch18` (nog te bouwen)
wanneer Minerva 18's overige stof (par. I-III abl.abs. met twee
naamwoorden, "slimme list") later een eigen hoofdstuk krijgt — die kan dan
niet meer op de ablativus absolutus zelf leunen, want die is al in H11
gedekt.

---

## Deel 2 (voorlopig vastgelegd, 2026-08-02) — Pallas 2 (les 22-29) & Minerva 2 (hoofdstuk 18-25)

**Vastgelegd in `SP_CAMPAIGN` (Chronica.md §7.45)** — Gerben ging akkoord
met het verdelingsvoorstel. Hoofdstuk 13 is inmiddels daadwerkelijk
GEBOUWD (2026-08-02, §7.47); Hoofdstuk 14-20 hebben nog enkel vastgelegde
metadata, geen scènes/puzzels (zie het onderscheid "vastgelegd" vs.
"gebouwd" dat deel 2's begin-tabel hierboven ook al aanhoudt).

| Pallas les | Onderwerp | Grammatica | Chronica H |
|---|---|---|---|
| 22 | Athena en Poseidon, Zeus en Metis, de geboorte van Athena | **Futurum** (bevat wél nieuwe grammatica, anders dan hier eerder aangenomen) — in Chronica ingezet via Tekst 22A (wedstrijd om Athene), NIET via Tekst 22B (Zeus/Metis-geboorte): die geboorte staat al volledig in Chronica Hoofdstuk 1, Lijn B; H13 zou hem anders dubbel vertellen — zie Chronica.md §7.47 (correctie) | 13 — **GEBOUWD** |
| 23 | De list van Peisistratos, de tirannendoders, kritiek op de Atheense democratie | *(cultuur/geschiedenis)* | 14 |
| 24 | Pandora, De Vrouw, mode | *(cultuur)* | 15 |
| 25 | Een slaaf wordt held, een slavin koopt zich vrij (slavernij) | *(cultuur)* | 15 (samen met 24) |
| 26 | Opvoeding, training, Plato's drie geslachten | *(cultuur)* | 16 |
| 27 | Sport, Olympische Spelen (Poulydamas, boksboete; incl. Atalanta, verhuisd vanuit het oude "De Stad van Athena") | *(cultuur)* | 16 (samen met 26) |
| 28 | Perzische Oorlogen (Thermopylae, Spartanen; Herodotos als vertellende stem) | *(cultuur/geschiedenis)* | 17 |
| 29 | Alexander de Grote | *(cultuur/geschiedenis — laatste Pallas 2-les)* | 20 — **laatste nieuwe Griekse stof in de hele campagne** (was 18, hernummerd 2026-08-09, tweede/chronologische correctie — zie Chronica.md §7.66) |
| — | **Ingevuld 2026-08-02**: de Diadochenoorlogen (Alexanders generaals verscheuren zijn rijk, direct vervolg op les 29) | — (herhaling, geen nieuwe les) | 22 (was 19, hernummerd 2026-08-09) |
| — | **Ingevuld 2026-08-02**: Cleopatra VII en de Slag bij Actium (31 v.Chr. — einde van het laatste Hellenistische rijk, inhoudelijk samenvallend met Augustus' Pax Romana aan de Latijnse kant) | — (herhaling, geen nieuwe les) | 23 (was 20, hernummerd 2026-08-09) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: Pericles/Delisch-Attische Zeebond/Parthenon-bouw | — (herhaling, geen nieuwe les) | 18 (nieuw, was 19 — hernummerd bij de chronologische correctie, §7.66) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: Peloponnesische Oorlog + Spartaanse/Thebaanse hegemonie | — (herhaling, geen nieuwe les) | 19 (nieuw, was 20 — hernummerd bij de chronologische correctie, §7.66) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: Griekse filosofie (Socrates/Plato/Aristoteles) | — (herhaling, geen nieuwe les) | 21 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: Griekse kunst/architectuur als vakgebied | — (herhaling, geen nieuwe les) | 24 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit), UITGEBREID met de presocraten** (§7.66): presocratische filosofie (Thales/Anaximander/Heraclitus/Pythagoras/Democritus) als opmaat, gevolgd door Hellenistische wetenschap (Archimedes/Euclides/Eratosthenes) | — (herhaling, geen nieuwe les) | 25 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: Grieks erfgoed onder Rome (Hadrianus' filhellenisme) | — (herhaling, geen nieuwe les) | 26 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit), UITGEBREID met Justinianus/Theodora** (§7.66): Byzantium wordt Constantinopel + korte vooruitblik naar Justinianus/Theodora | — (herhaling, geen nieuwe les) | 27 (nieuw, zigzag-hoofdstuk) |

| Minerva H | Onderwerp | Grammatica | Chronica H |
|---|---|---|---|
| 18 | Hannibal verslagen (hebzucht Kretenzers, slimme list) | Ablativus absolutus — **al gedekt in H11** (2026-08-02 gecorrigeerd, zie boven); resterende cultuurstof nog te gebruiken | 17 (samen met Pallas 28) |
| 19 | Dagelijks leven in Rome | Vragend voornaamwoord, onbepaald voornaamwoord, perfectum met praesensbetekenis | 15 (samen met Pallas 24-25) |
| 20 | Vermaak in Rome | Velle/nolle/fieri, betrekkelijke bijzin met ingesloten antecedent, comparativus/superlativus | 16 (samen met Pallas 26-27) |
| 21 | De Romeinen en de dood (grafschriften) | Conjunctivus praesens + gebruik, conjunctivus perfectum | 13 — **GEBOUWD** (2026-08-02, opgelost gat: ingezet via Brutus' eed over Lucretia en de twijfel over Horatius' daad, i.p.v. via het eigen "grafschriften"-cultuurtekst — decoupling-beleid, zie Chronica.md §7.47) |
| 22 | Caesars carrière (Belgen bij Bibrax, Dumnorix) | Gerundium, genitivus subjectivus/objectivus, *ferre* | **17** (VERVROEGD vanuit 18, 2026-08-09 — sterke klik met Alpenoversteek/beleg van Saguntum bij Hannibal; cultuurtekst Caesar-in-Gallië/Britannia blijft wel bij Chronica H20, was H18 — hernummerd bij de chronologische correctie, §7.66) |
| 23 | Caesar in België en Nederland (Nervii, rivaliteit) | Gerundivum, gerundivumconstructie | **17** (idem, vervroegd) |
| 24 | Keizers van Rome: Augustus en Nero | N.C.I. (Nominativus cum Infinitivo), semi-deponentia, transitief/intransitief, vraagpartikels | 23 — **GEBOUWD** (2026-08-23, post-build audit §7.107; stond tot dan alleen in de metadata en kwam nergens in het spel voor). Vier codextabellen + overzicht, drie puzzels (N.C.I. op Augustus' `rem publicam restituisse dicitur`, semi-deponens `ausus est` op Tiberius/Capri, `num` op de moord op Caligula) en een leesval `nonne` versus `num` op Seneca's sterfbed. Lopende saves krijgen de codexpagina's via `SP_PATCHES`-entry `h23-minerva24-grammatica`. |
| 25 | Hartstocht en spanning (Catullus, Petronius) | *(vnl. vertaalvaardigheid, weinig nieuwe grammatica — geschikte afsluiter)* | 23 (samen met M24, was 20) |
| — | **Ingevuld 2026-08-09 (KCV-audit, chronologische correctie §7.66)**: Gracchen → Marius (legerhervorming, Romeins leger als systeem) → Sulla | — (herhaling, geen nieuwe les) | 18 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit, chronologische correctie §7.66)**: Caesars Burgeroorlog (Rubicon–Pharsalus–Iden van maart) | — (herhaling, geen nieuwe les) | 19 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit, chronologische correctie §7.66)**: Cicero (redevoeringen, vertaler van Griekse filosofie) + Seneca-introductie (payoff bij Nero, H23) | — (herhaling, geen nieuwe les) | 21 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit, chronologische correctie §7.66)**: Tweede Triumviraat/Antonius' opkomst | — (herhaling, geen nieuwe les) | 22 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: Romeinse architectuur/techniek | — (herhaling, geen nieuwe les) | 24 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: Pompeii (aardbeving 62, uitbarsting 79, Plinius) | — (herhaling, geen nieuwe les) | 25 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit)**: volledig keizersoverzicht Flavii t/m Crisis 3e Eeuw + Bataafse Opstand/Romeins Nederland | — (herhaling, geen nieuwe les) | 26 (nieuw) |
| — | **Ingevuld 2026-08-09 (KCV-audit, UITGEBREID §7.66)**: Constantijn/christendom, deling Rijk, val West-Rome (476) | — (herhaling, geen nieuwe les) | 27 (nieuw, zigzag-hoofdstuk) |

**Let op, nog open**: Minerva 21 (grafschriften/dood) kreeg geen plek in de
uiteindelijke 8-hoofdstukindeling — bij het uitschrijven bleek Hoofdstuk
19/20 al gevuld met Gracchen/Marius-Sulla resp. Augustus/Nero/literatuur
als CULTUURTEKST, zonder dat Minerva 21's eigen grammatica daar nog bij
paste. Te bepalen: ofwel Minerva 21 alsnog aan Hoofdstuk 19 of 20 toevoegen
(check eerst of dat niet te veel puzzels wordt), ofwel bewust laten
vervallen (er is al meer dan genoeg Minerva-stof voor de resterende
hoofdstukken).

**Belangrijk verschil met deel 1**: Minerva's hoofdstukvolgorde in deel 2
volgt NIET de eigen Romeinse geschiedenis chronologisch (H15 is een
Bijbelverhaal, H18-25 springen qua onderwerp door elkaar heen) — het is een
opeenvolging van cultuurteksten gekozen om grammaticale progressie te
ondersteunen, niet chronologie. Chronica hoeft de Minerva-hoofdstukvolgorde
dus net zomin te volgen als de eigen verhaalvolgorde (Kings → Republiek →
Gracchen → Caesar → Augustus) — precies zoals bij deel 1 al gebeurde.

**Asymmetrie om in de gaten te houden**: Pallas 2 eindigt bij les 29
(Alexander de Grote, 323 v.Chr.) — daarna is er geen nieuwe Griekse
grammatica meer. Minerva 2 loopt door tot hoofdstuk 25 (Nero, 1e eeuw
n.Chr.), dus zeker vier hoofdstukken verder. Vanaf het punt waar Pallas
"op" is, moet de Griekse lijn overschakelen op bewuste herhaling/lichte
aanwezigheid (zoals Hoofdstuk 5-7 dat nu al voor Latijn doen) — nooit
volledig afwezig, dat zou weer de "geen enkelvoudig-spoor-hoofdstukken"-
regel schenden (zie `chronica-geen-enkelvoudig-spoor-hoofdstukken` in het
geheugensysteem).

---

## Hoe dit document bij te werken

Bij elke volgende toewijzingsronde (wanneer Chronica H13+ definitief wordt
vastgelegd in `SP_CAMPAIGN`): werk de laatste tabel hierboven bij door de
"voorstel"-kolom te vervangen door de definitieve keuze, en verplaats de
rij naar de "Deel 2 (begonnen)"-tabel zodra de puzzels daadwerkelijk
gebouwd zijn (niet al bij het vastleggen van de metadata — zie het
onderscheid tussen "vastgelegd" en "gebouwd" dat Chronica.md consequent
aanhoudt, bv. §7.43).
