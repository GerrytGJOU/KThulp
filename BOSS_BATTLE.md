...
| Raster-opstelling op het slagveld | `battle.js`: `bmGridSlots`/`bmFormationHTML`, `.bm-gridform` in `index.html` | Dezelfde RPG Maker MV-achtige rasteropstelling als Battle Mode (zie BATTLE_MODE.md §Formatie-indeling). Omdat de hele klas hier op één helft staat, gaat het raster 5 rijen diep in plaats van 4 (`BM_GRID_ROWS_BOSS`, dezelfde drempel geldt voor een Battle Mode-team van meer dan 16), krijgt helft A meer breedte (`#bmField.bm-boss #bmFormA{flex:1.7}`) en zet `bmFormationHTML()` de rijen via `--rg` dichter op elkaar; tot 32 spelers passen zo naast de baas. De baas zelf blijft een losse `.bm-fcol`-kolom in `#bmFormB` (`bmBossSpriteHTML()`), maar wordt met `.bm-boss-art` (`clamp(200px,26vw,460px)`) véél groter getekend dan een speler — op 168×168 was de Hydra even groot als één leerling. Naam en fase-regel schalen mee. |
